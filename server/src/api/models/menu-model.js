...
const default_menu = {
  ...
  message:
    "default menu. meals array contains meal.id values. date format: YYYY-MM-DD",
};
    ...
    if (menuArray) {
      const returnArray = await Promise.all(
        menuArray.map(async (menu) => {
          const [mealArray] = await promisePool.query(
            `SELECT meal FROM menu_meals WHERE menu_meals.menu = ${menu.id}`
          );
...
const findMenuById = async (id) => {
  try {
    ...
    const query = promisePool.format(
      "SELECT * FROM menus where menus.id = ?",
      id
    );
  ...
  try {
    ...
    try {
      ...
      const newMenu = {
        ...default_menu,
        ...menu,
        message: "new menu added by menu-model",
      };
...
const modifyMenu = async (menu, menuId) => {
  try {
    ...
    const updatedMenu = {
      ...previousMenu,
      ...menu,
      message: "menu modified by menu-model",
    };
...
const findMenusByDate = async (date) => {
  try {
    ...
    const query = promisePool.format(
      "SELECT * FROM menus where menus.date = ?",
      date
    );
    const [menuArray] = await promisePool.execute(query);
    ...
    if (menuArray.length > 0) {
      ...
      console.log(
        "return menus in findMenusByDate in menu-model:",
        returnArray
      );
      return returnArray;
    } else {
...
const findMenusByWeek = async (week) => {
  try {
    ...
    const query = promisePool.format(
      "SELECT * FROM menus where menus.week = ?",
      week
    );
    const [menuArray] = await promisePool.execute(query);
    ...
    if (menuArray.length > 0) {
      ...
      console.log(
        "return menus in findMenusByWeek in menu-model:",
        returnArray
      );
      return returnArray;
    } else {
