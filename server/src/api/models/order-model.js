    ...
    try {
        ...
        try {
            ...
            if (orderResult[0].insertId && orderResult[0].affectedRows > 0 ) {
                ...
                const formatted_res = connection.format(reservationSql , [reservationParams]);
                ...
                //if results
                if (resResult && gcResult) {  //TODO: better error checking
                    //return added order
    ...
    try {
        ...
        /*    //disable foreign key checks
            ...
            if (orderResult[0].affectedRows > 0 ) {
                ...
                const resParams = [];
                updatedOrder.reservations.forEach((resId) => {resParams.push( [updatedOrder.id, resId] );});
                ...
                const gcParams = [];
                updatedOrder.gift_cards.forEach( gcId => gcParams.push( [updatedOrder.id, gcId] ) );
...
const removeOrder = async (orderId) => {
    try {
        ...
        try {
            ...
            const resSql = "DELETE FROM order_reservations WHERE order_reservations.order = ?";
            const resParams = [orderId];
            const [resResult] = await connection.execute(resSql, resParams);

            //delete from order_gift_cards table
            const gcSql = "DELETE FROM order_gift_cards WHERE order_gift_cards.order = ?";
...
const findOrdersByTimestamp = async (date) => {
    try {
        ...
        const query = promisePool.format('SELECT * FROM orders where orders.timestamp = ?', userId);
        const [orderArray] = await promisePool.execute(query);
        ...
        if (orderArray.length > 0) {
            //get arrays for all orders
            const returnArray = await Promise.all( orderArray.map( order => getOrderSubArrays(order) ));
            console.log('return order in findOrdersByDate in order-model:',returnArray[0]);
            return returnArray[0];
