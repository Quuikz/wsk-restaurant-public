...
const findOrdersByLocation = async (locationId) => {
    try {
        const resultArray = orders.filter(order => order.location === Number(locationId));
        if (resultArray) {
            return resultArray;
        } else {
            return false
...
export {listAllOrders, findOrderById, addOrder, modifyOrder, removeOrder, findOrdersByUserId, findOrdersByLocation, findOrdersByDate};
